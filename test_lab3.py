...
def test_mean_fare_survived_S(titanic_df):
    result = mean_fare(titanic_df, "S", True)
    assert result == 10


def test_mean_fare_dead_S(titanic_df):
    result = mean_fare(titanic_df, "S", False)
    assert result == 20


def test_mean_fare_survived_C(titanic_df):
    result = mean_fare(titanic_df, "C", True)
    assert result == 30


def test_mean_fare_dead_Q(titanic_df):
    result = mean_fare(titanic_df, "Q", False)
    assert result == 60


def test_mean_fare_nan(titanic_df):
    s = mean_fare(titanic_df, "X", True)
    d = mean_fare(titanic_df, "X", False)
    assert np.isnan(s)
    assert np.isnan(d)
